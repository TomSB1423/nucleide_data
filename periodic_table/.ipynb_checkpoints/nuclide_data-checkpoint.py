...
mev_per_c_2_amu = 1.0 / 931.494061
...
z2sym = dict([(Z, nist_per_element[Z][0]["Atomic Symbol"]) for Z in range(1, 119)])
...
def parse_one_wallet_line(line):
    ...
    d["abundance"] = do_if_present(line[81:96], process_abundance, default=0.0)

    d["Jpi"] = line[16:26].strip()

    d["isomeric"] = line[4] == "M"
    d["excitation energy"] = do_if_present(line[42:49], lambda x: float(x), default=0.0)
...
for el in wallet_nuclide_processed_list:
    ...
    if not (E in isomers):
        ...
        if isomer["half-life"] == 0.0:
            isomer["lambda"] = np.inf
        else:
            isomer["lambda"] = np.log(2.0) / isomer["half-life"]
...
def weight(Z_or_symbol, A=None, E=0.0):
    """
...
class Nuclide:
    ...
    def __init__(self, nuc_id, E=0.0, metastable=False):
            ...
            try:
                ...
                if type(nuc_id) is str:
                    ...
                    if re.search("[a-zA-Z]", nuc_id):
                        ...
                        # Metastable
                        if nuc_id[0] in string.ascii_letters and nuc_id[-1] == "M":
                            metastable = True
                            nuc_id = nuc_id[:-1]
                        ...
                        if re.search("-", nuc_id):
                            ...
                            s1 = "".join(
                                filter(lambda x: x in string.ascii_letters, nuc_id)
                            )
                            s2 = "".join(
                                filter(
                                    lambda x: not (x in string.ascii_letters), nuc_id
                                )
                            ).strip()
        ...
        if metastable and E == 0.0:
            E = np.inf

        self.metastable = E > 0.0
        ...
        if self.E is np.inf and self.__repr__() in default_isomer_E.keys():
